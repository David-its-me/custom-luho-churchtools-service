...
def generate_cue_group_from_cues(
    cues: list[Cue],
    color: Color = luhovina(),
    label: str = "",
...
def createCue(slide: Slide, actions: list[Action] = [], completion_time: float = 0.0) -> Cue:
    ...
    if completion_time > 0:
        ...
        cue.completion_action_type = (
            Cue.CompletionActionType.COMPLETION_ACTION_TYPE_AFTER_TIME
        )
        # completion_action_type=Cue.CompletionActionType.COMPLETION_ACTION_TYPE_LAST,
        cue.completion_target_type = (
            Cue.CompletionTargetType.COMPLETION_TARGET_TYPE_NEXT
        )
